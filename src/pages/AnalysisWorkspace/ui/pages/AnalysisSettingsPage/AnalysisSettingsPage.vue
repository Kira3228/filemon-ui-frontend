...
import { useGetDatabaseSettings, DATABASE_SETTINGS_QUERY_KEY } from "../../../model/queries/useGetDatabaseSettings";
import { ANALYSIS_REPORT_QUERY_KEY, useGetAnalysisReport } from "../../../model/queries/useGetAnalysisReport";
...
const {
  ...
  isFetching: isDatabaseSettingsFetching,
  isLoading: isDatabaseSettingsLoading,
  refetch: refetchDatabaseSettings,
} = useGetDatabaseSettings();
const { data: report } = useGetAnalysisReport();
const {
  autoRefreshEnabled,
...
const databaseLoading = computed(
  () => isDatabaseSettingsLoading.value || isDatabaseSettingsFetching.value,
);
...
const loadDatabaseSettings = async () => {
  ...
  try {
    const result = await refetchDatabaseSettings();
    if (result.data) {
      applyDatabaseSettings(result.data);
    }
    if (result.error) {
      throw result.error;
    }
...
const saveAllSettings = async () => {
  ...
  try {
    if (isDatabasePathDirty.value) {
      const settings = await DatabaseService.updateState({
        databasePath: nextDatabasePath,
      });

      applyDatabaseSettings(settings);
      queryClient.setQueryData(DATABASE_SETTINGS_QUERY_KEY, settings);
      await reloadReportForDatabaseChange();
      await queryClient.invalidateQueries({ queryKey: ANALYSIS_REPORT_QUERY_KEY });
    }
...
watch(
  ...
  (error) => {
    ...
    settingsErrorMessage.value =
      error instanceof Error
        ? error.message
        : "РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ РЅР°СЃС‚СЂРѕР№РєРё Р±Р°Р·С‹ РґР°РЅРЅС‹С….";
  },
  { immediate: true },
);
</script>
